...
def _(charges):
    for charge in charges:
        tax_ = charge * 0.625
        cost_ = charge + tax_
        print(f'pre-tax: ${charge:.2f}, tatal: ${cost_:.2f}')
    return


@app.cell
def _(charges):
    # sum up all the charges that are undder $25
    sum_below_25 = 0
    for charge_below_25 in charges:
        if charge_below_25 < 25:
            sum_below_25 = sum_below_25 + charge_below_25

    print(f'Total charges below $25: {sum_below_25:.2f}')
    return
...
def _(mo):
    mo.md(r"""
    when a score satisfies two of these tests at once, which one decides what gets printed?
    A.The first would decide. *note the function of if and elif, there could be two results if we change elif into if
    """)
    return
